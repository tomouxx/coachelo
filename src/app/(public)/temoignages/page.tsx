...
export default async function TemoignagesPage() {
  const list = await prisma.testimonial.findMany({ where: { published: true }, orderBy: { sortOrder: "asc" } }).catch(() => []);

  return (
    <>
      <section className="container-editorial pt-20 pb-12 text-center">
        <p className="eyebrow">Témoignages</p>
        <h1 className="section-title mt-3">Leurs transformations</h1>
        <p className="mt-5 max-w-xl mx-auto text-brand-taupe">
          Ils m'ont fait confiance. Voici leurs mots.
        </p>
      </section>
      ...
      <section className="container-editorial pb-24 columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
        {list.map((t) => (
          <figure
            key={t.id}
            className="break-inside-avoid bg-brand-nude rounded-xl2 p-7 shadow-card"
          >
